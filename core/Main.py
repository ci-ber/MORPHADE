...
def add_args(parser):
    ...
    parser.add_argument('--config_path', type=str, metavar='C',
...
if __name__ == "__main__":
    arg_parser = add_args(argparse.ArgumentParser(description='MORPHADE'))
    ...
    config_file = None
    logging.info(
        '------------------------------- DEEP LEARNING FRAMEWORK *MORPHADE*  -------------------------------')
    try:
        stream_file = open(args.config_path, 'r')
        config_file = yaml.load(stream_file, Loader=yaml.FullLoader)
        logging.info('[MORPHADE::main] Success: Loaded configuration file at: {}'.format(args.config_path))
    except:
        logging.error('[MORPHADE::main] ERROR: Invalid configuration file at: {}, exiting...'.format(args.config_path))
        exit()
